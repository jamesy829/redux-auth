...
export const UPDATE_PASSWORD_MODAL_FORM_UPDATE =
...
export function updatePasswordModal(body, endpointKey) {
  return dispatch => {
    ...
    return fetch(getPasswordUpdateUrl(endpointKey), {
      ...
      .then(parseResponse)
      .then(({ user }) =>
        dispatch(updatePasswordModalComplete(endpointKey, user))
      )
      .catch(({ errors }) =>
        dispatch(updatePasswordModalError(endpointKey, errors))
      );
  };
}
