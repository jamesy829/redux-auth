...
export function requestPasswordReset(body, endpoint) {
  return dispatch => {
    ...
    return fetch(getPasswordResetRequestUrl(endpoint), {
      ...
      .then(parseResponse)
      .then(({ message }) =>
        dispatch(requestPasswordResetComplete(endpoint, message))
      )
      .catch(({ errors }) =>
        dispatch(requestPasswordResetError(endpoint, errors))
      );
  };
}
