...
function mapStateToProps({ auth }, ownProps) {
  const endpoint = (ownProps.endpoint ||
...
const handleFormErrors = err => {
  if (err && err.errors) {
    const errorStr = typeof(err.errors) === 'Array' ? err.errors.join(' ') : err.errors;
...
function mergeProps(stateProps, dispatchProps, ownProps) {
  return Object.assign({}, ownProps, stateProps, dispatchProps, {
    onSubmit: formData => {
      return dispatchProps.emailSignIn(formData.toJS(), stateProps.endpoint)
        .then(ownProps.next)
...
export default connect(mapStateToProps, { emailSignIn }, mergeProps)(reduxForm({})(EmailSignInFormView));
