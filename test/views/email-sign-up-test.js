      ...
      describe(`${theme} theme`, () => {
        describe(`params`, () => {
          it("should accept styling params", done => {
            let classNameProp =
              theme === "/views/bootstrap" ? "groupClassName" : "className";

            let inputProps = {
              email: {
                style: { color: "red" },
                [classNameProp]: "email-class-override"
              },
              password: {
                style: { color: "green" },
                [classNameProp]: "password-class-override"
              },
              passwordConfirmation: {
                style: { color: "purple" },
                [classNameProp]: "password-confirmation-class-override"
              },
            ...
            renderConnectedComponent(
              ...
              .then(({ instance }) => {
                ...
                let passwordConfirmationEl = findClass(
                  instance,
                  "password-confirmation-class-override"
                );
                findClass(instance, "submit-class-override");
                expect(emailEl.getAttribute("style")).to.match(/color:\s?red/);
                expect(passwordEl.getAttribute("style")).to.match(
                  /color:\s?green/
                );
                expect(passwordConfirmationEl.getAttribute("style")).to.match(
                  /color:\s?purple/
                );
                done();
              })
        ...
        describe("success", () => {
          beforeEach(() => {
            ...
            successRespSpy = sinon.spy(url => {
              return mockFetchResponse(
                url,
                200,
                { data: { uid: testUid, email: testUid } },
                successRespHeaders
              );
            });
          });
          ...
          it("should handle successful sign up", done => {
            ...
            renderConnectedComponent(<EmailSignUpForm next={nextSpy} />, {
              apiUrl
            })
              .then(({ instance, store }) => {
                let emailEl = findTag(instance, "input")[0];
                let passwordEl = findTag(instance, "input")[1];
                let passwordConfirmEl = TestUtils.scryRenderedDOMComponentsWithTag(
                  instance,
                  "input"
                )[2];
                ...
                expect(
                  store
                    .getState()
                    .auth.getIn(["emailSignUp", "default", "form", "email"])
                ).to.equal(testEmail);
                expect(
                  store
                    .getState()
                    .auth.getIn(["emailSignUp", "default", "form", "password"])
                ...
                setTimeout(() => {
                  // ensure user was set
                  let sentAddress = store
                    .getState()
                    .auth.getIn(["ui", "emailSignUpAddress"]);
                  expect(sentAddress).to.equal(testEmail);

                  // ensure success modal is present
                  let modalVisible = store
                    .getState()
                    .auth.getIn(["ui", "emailSignUpSuccessModalVisible"]);
                  ...
                  const uid = store
                    .getState()
                    .auth.getIn(["user", "attributes", "uid"]);
          ...
          it("should sign the user in if registration returns user data", done => {
            ...
            renderConnectedComponent(<EmailSignUpForm />, { apiUrl }).then(
              ({ instance, store }) => {
                let emailEl = findTag(instance, "input")[0];
                let passwordEl = findTag(instance, "input")[1];
                let passwordConfirmEl = TestUtils.scryRenderedDOMComponentsWithTag(
            ...
            renderConnectedComponent(<EmailSignUpForm next={nextSpy} />, {
              ...
              .then(({ instance, store }) => {
                ...
                setTimeout(() => {
                  ...
                  let errors = store
                    .getState()
                    .auth.getIn(["emailSignUp", "default", "errors"])
                    .toJS();
                  ...
                  let user = store
                    .getState()
                    .auth.getIn(["user", "attributes"]);
                  expect(user).to.equal(null);

                  // ensure modal is to be shown
                  let modalVisible = store
                    .getState()
                    .auth.getIn(["ui", "emailSignUpErrorModalVisible"]);
                  expect(modalVisible).to.equal(true);

                  // ensure errors show up in form
                  let errorItems = TestUtils.scryRenderedDOMComponentsWithClass(
                    instance,
                    "inline-error-item"
                  );
