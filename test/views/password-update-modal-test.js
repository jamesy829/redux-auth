...
var findClass = className => document.getElementsByClassName(className)[0],
  findButtonWithType = type =>
    [].slice.call(document.getElementsByTagName("button")).filter(el => {
      ...
      describe(`${theme} theme`, () => {
        describe(`params`, () => {
          it("should accept styling params", done => {
            ...
            let classNameProp =
              theme === "bootstrap" ? "groupClassName" : "className";
            ...
            renderConnectedComponent(
              <PasswordResetSuccessModal inputProps={inputProps} show={true} />,
              undefined,
              initialState
            )
              .then(() => {
                expect(findClass("password-class-override")).to.be.ok;
                expect(findClass("password-confirmation-class-override")).to.be
                  .ok;
            ...
            renderConnectedComponent(
              ...
              .then(({ store }) => {
                ...
                setTimeout(() => {
                  ...
                  let errors = store
                    .getState()
                    .auth.getIn(["updatePasswordModal", "default", "errors"])
                    .toJS();
                  ...
                  let modalVisible = store
                    .getState()
                    .auth.getIn(["ui", "passwordResetSuccessModalVisible"]);
                  expect(modalVisible).to.equal(true);

                  // ensure errors show up in form
                  let errorItems = document.getElementsByClassName(
                    "inline-error-item"
                  );
