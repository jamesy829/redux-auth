...
export default function() {
  describe("EmailSignInForm", () => {
    ["/views/default", "/views/material-ui", "/views/bootstrap"].forEach(
      theme => {
        ...
        describe(`${theme || "default"} theme`, () => {
          describe(`params`, () => {
            it("should accept styling params", done => {
              ...
              let inputProps = {
                ...
                submit: {
                  style: { color: "orange" },
                  className: "submit-class-override"
                }
              };
              ...
              renderConnectedComponent(
                ...
                .then(({ instance }) => {
                  ...
                  let passwordEl = findClass(
                    instance,
                    "password-class-override"
                  );
                  findClass(instance, "submit-class-override");
                  expect(emailEl.getAttribute("style")).to.match(
                    /color:\s?red/
                  );
              ...
              renderConnectedComponent(
                ...
                .then(({ instance, store }) => {
                  ...
                  setTimeout(() => {
                    ...
                    expect(authHeaders["access-token"]).to.equal(
                      successRespHeaders["access-token"]
                    );

                    // ensure default endpoint key was set to "default"
                    let defaultEndpoint = store
                      .getState()
          ...
          describe(`success`, () => {
            it("should handle successful sign in", done => {
              ...
              renderConnectedComponent(<EmailSignInForm next={nextSpy} />, {
                apiUrl
              })
                .then(({ instance, store }) => {
                  // find inputs
                  let emailEl = TestUtils.scryRenderedDOMComponentsWithTag(
                    instance,
                    "input"
                  )[0];
                  let passwordEl = TestUtils.scryRenderedDOMComponentsWithTag(
                    instance,
                    "input"
                  )[1];
                  ...
                  expect(
                    store
                      .getState()
                      .auth.getIn(["emailSignIn", "default", "form", "email"])
                  ).to.equal(testEmail);
                  expect(
                    store
                      .getState()
          ...
          describe(`error`, () => {
            it("should handle failed sign in", done => {
              ...
              renderConnectedComponent(<EmailSignInForm next={nextSpy} />, {
                ...
                .then(({ instance, store }) => {
                  // submit the form
                  let formEl = TestUtils.findRenderedDOMComponentWithClass(
                    instance,
                    "email-sign-in-form"
                  );
                  TestUtils.Simulate.submit(formEl, { preventDefault: spy() });
                  ...
                  setTimeout(() => {
                    ...
                    let errors = store
                      .getState()
                      .auth.getIn(["emailSignIn", "default", "errors"])
                      .toJS();
                    ...
                    let modalVisible = store
                      .getState()
                      .auth.getIn(["ui", "emailSignInErrorModalVisible"]);
                    expect(modalVisible).to.equal(true);
