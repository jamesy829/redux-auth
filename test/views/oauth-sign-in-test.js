...
export default function() {
  describe("OAuthSignInButton", () => {
    function popupSuccessMock(provider, targetUrl) {
      ...
      setTimeout(() => {
        let params = `token=${
          popupSuccessParams["access-token"]
        }&uid=${encodeURIComponent(testUid)}`;
    ...
    ["bootstrap", "material-ui", "default"].forEach(theme => {
      var OAuthSignInButton = rewire(
        `../../src/views/${theme}/OAuthSignInButton`
      );
      ...
      describe(`${theme} theme`, () => {
        describe("params", () => {
          ...
          it("should allow the use of alternate endpoints", done => {
            ...
            nock(apiUrl)
              ...
              .matchHeader(
                "access-token",
                ([h]) => h === popupSuccessParams["access-token"]
              )
            ...
            renderConnectedComponent(
              ...
              .then(({ instance, store }) => {
                // click button
                let submitEl = TestUtils.findRenderedDOMComponentWithTag(
                  instance,
                  "button"
                );
        ...
        describe("success", () => {
          it("retrieves auth creds from external oauth login window, makes validation request to API", done => {
            ...
            renderConnectedComponent(
              <OAuthSignInButton.default next={nextSpy} provider="github" />,
              { apiUrl }
            )
              .then(({ instance, store }) => {
                // click button
                ...
                setTimeout(() => {
                  ...
                  expect(currentCreds["access-token"]).to.equal(
                  ...
                  let currentUser = store.getState().auth.get("user");
                  expect(currentUser.get("isSignedIn")).to.equal(true);
                  expect(currentUser.getIn(["attributes", "uid"])).to.equal(
                    testUid
                  );
                  ...
                  expect(
                    store
                      .getState()
                      .auth.getIn(["ui", "oAuthSignInSuccessModalVisible"])
                  ).to.equal(true);
              ...
              .then(({ instance, store }) => {
                ...
                setTimeout(() => {
                  ...
                  expect(
                    store
                      .getState()
                      .auth.getIn(["ui", "oAuthSignInErrorModalVisible"])
                  ).to.equal(true);
                  done();
                }, 100);
              })
